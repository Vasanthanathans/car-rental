...
class GlobalFunction extends Model
{
    ...
    public static function sendPushNotificationToUsers($title, $message)
    {
        ...
        $fields = array(
            'to' => '/topics/users', 'notification' => $notificationArray, 'priority' => 'high'
        );
        $headers = array(
    ...
    public static function sendPushNotificationToSalons($title, $message)
    {
        ...
        $fields = array(
            'to' => '/topics/salons', 'notification' => $notificationArray, 'priority' => 'high'
        );
        $headers = array(
            'Content-Type:application/json',
            'Authorization:key=' . $api_key
    ...
    public static function sendPushToSalon($title, $message, $salon)
    {
        ...
        if ($salon->is_notification == 1) {
            $fields = array('to' => "/token/" . $salon->device_token, 'notification' => $notificationArray, 'priority' => 'high');
            $headers = array(
                'Content-Type:application/json',
                'Authorization:key=' . $api_key
            );
    ...
    public static function saveFileAndDynamicPath($file, $path)
    {
        if ($file != null) {
    ...
    public static function createDirecotyFiles($directory, $fileName, $data)
    {
        // Laravel base path
        $basePath = base_path();
